...
public class RollingDie extends Die
{
  ...
  private void drawDots(Graphics g, int x, int y, int numDots)
  {
    ...
    // Use the method: g.fillOval(x2, y2, dotSize, dotSize);
      ...
      //<Add code here>
    /*This nested if structure is the most efficient way to execute this piece of code 
     * that I could think of.
     * It is rather complicated so I have added comments for clarity */
   
    // fills the middle dot if it is odd, all odd numbers have filled center
    if (numDots == 1 || numDots == 3 || numDots == 5){
    	...
    	if (numDots >= 3)
    	{
        	...
        	// fills the top left and bottom right corners if the number is three or five as both have filled corners
        	if (numDots ==5){
        		g.fillOval(x1, y3, dotSize, dotSize);
            	g.fillOval(x3, y1, dotSize, dotSize);
            	//fills the remaining corners for five
        	}
    	...
    	g.fillOval(x1, y1, dotSize, dotSize);
    	g.fillOval(x3, y3, dotSize, dotSize);
    	// fills the top left and bottom right if it is even, all odd numbers have these dots filled
    	if (numDots >= 4){
    		g.fillOval(x1, y3, dotSize, dotSize);
        	g.fillOval(x3, y1, dotSize, dotSize);
        	//as both four and six have other corners filled, and odds are excluded it is safe to fill other corners
        	if (numDots == 6){
        		g.fillOval(x2, y3, dotSize, dotSize);
        		g.fillOval(x2, y1, dotSize, dotSize);
        		//fills the middle row for six
        	}
    	}
    }
  }
